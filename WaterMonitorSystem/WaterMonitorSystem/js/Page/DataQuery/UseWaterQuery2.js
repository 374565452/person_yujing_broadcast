...
function QueryCurrentPageData(pageNumber, pageSize) {
    ...
    var colflag = false;
    $.ajax({
        url: "../WebServices/DataQueryService.asmx/GetEventRecords2",
            ...
            if (data.Result) {
                ...
                for (var i = 0; i < DevDatas.length; i++) {
                    ...
                    for (var item in DevDatas[i]) {
                        ...
                        if (colflag) {
                            var colWidth = 100;
                            if (item == "事件类型" || item == "事件时间" || item == "设备编号" || item == "开泵时间" || item == "关泵时间" || item == "灌溉时长") {
                                colWidth = 120;
                            }
...
function Btn_Excel_Click() {
    ...
    $.ajax(
    {
        url: "../WebServices/DataQueryService.asmx/ExportEventRecords2",
        type: "GET",
