        ...
        success: function (responseText) {
            ...
            if (data.Result == true) {
                    ...
                    for (var j = 0; j < xAxis_categories.length; j++) {
                        ...
                        for (var k = 0; k < data.Records.length; k++) {
                            if (data.Records[k].Acq_Time == xAxis_categories[j] && data.Records[k].StationID == series_name[i]) {
                                myMap1['data'].push(parseFloat(data.Records[k].Rainfall));
                                flag = true;
                                break;
                            }
                        }
                ...
                $('#divChart1').highcharts({
                    ...
                    yAxis: { title: { text: '雨量（mm）' }, labels: { formatter: function () { return this.value + 'mm' } } },
...
function QueryCurrentPageData(pageNumber, pageSize) {
    var startIndex = (pageNumber - 1) * pageSize + 1;
    $.ajax({
        url: "../WebServices/DataQueryNewService.asmx/GetRainFallRecords",
        data: { "loginIdentifer": window.parent.guid, "operateIdentifer": operateIdentifer, "startIndex": startIndex, "count": pageSize },
        type: "GET",
        dataType: "text",
        cache: false,
        success: function (responseText) {
            ...
            if (data.Result == true) {
                ...
                for (var i = 0; i < Records.length; i++) {
                    var tableRow = {};
                    tableRow["StationID"] = Records[i].StationID;
                    tableRow["Rainfall"] = Records[i].Rainfall;
                    tableRow["Rainfall_Total"] = Records[i].Rainfall_Total;
                    if (Records[i].WaterLevel1 == "三级") {
                        tableRow["WaterLevel1"] = "" + Records[i].WaterLevel1 + " <span style=\"background-color:#FFFF00; padding:10px 20px;\"></span>";
                    } else if (Records[i].WaterLevel1 == "二级") {
                        tableRow["WaterLevel1"] = "" + Records[i].WaterLevel1 + " <span style=\"background-color:#FFA500; padding:10px 20px;\"></span>";
                    } else if (Records[i].WaterLevel1 == "一级") {
                        tableRow["WaterLevel1"] = "" + Records[i].WaterLevel1 + " <span style=\"background-color:#FF0000; padding:10px 20px;\"></span>";
                    } else {
                        tableRow["WaterLevel1"] = Records[i].WaterLevel1;
                    }
                    tableRow["BV"] = Records[i].BV;
                    tableRow["Acq_Time"] = Records[i].Acq_Time;
...
function Btn_Excel_Click() {
    ...
    $.ajax(
    {
        url: "../WebServices/DataQueryNewService.asmx/ExportRainFallRecords",
        type: "GET",
