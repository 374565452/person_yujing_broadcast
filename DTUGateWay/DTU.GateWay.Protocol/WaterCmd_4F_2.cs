...
{
    public class WaterCmd_4F_2 : WaterBaseMessage
    {
        public WaterCmd_4F_2()
        {
            AFN = (byte)WaterBaseProtocol.AFN._4F;
        ...
        public string ReadMsg(string UserDataAll)
        {
            ...
            try
            {
                RC = Convert.ToByte(UserDataAll.Substring(30, 2), 16);
            }
            catch (Exception ex)
            {
                if (ShowLog) logHelper.Error(ex.Message + Environment.NewLine + "获取定值控制出错" + " " + RawDataStr);
                return "获取定值控制出错";
            }
