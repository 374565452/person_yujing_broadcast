...
{
    public class WaterCmd_21_1 : WaterBaseMessage
    {
        public WaterCmd_21_1()
        {
            AFN = (byte)WaterBaseProtocol.AFN._21;
        ...
        public string ReadMsg()
        {
            ...
            try
            {
                AlarmType = Convert.ToByte(UserData.Substring(16, 2), 16);
            }
            catch (Exception ex)
            {
                if (ShowLog) logHelper.Error(ex.Message + Environment.NewLine + "获取预警类型出错" + " " + RawDataStr);
                return "获取预警类型出错";
            }

            try
            {
                AlarmValue = Convert.ToByte(UserData.Substring(18, 2), 16);
            }
            catch (Exception ex)
            {
                if (ShowLog) logHelper.Error(ex.Message + Environment.NewLine + "获取预警级别出错" + " " + RawDataStr);
                return "获取预警级别出错";
            }
