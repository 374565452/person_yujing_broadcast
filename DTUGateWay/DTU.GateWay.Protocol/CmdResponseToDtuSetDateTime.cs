...
{
    public class CmdResponseToDtuSetDateTime : BaseMessage
    {
        public CmdResponseToDtuSetDateTime()
        {
        ...
        public CmdResponseToDtuSetDateTime(BaseMessage bm)
        {
            BeginChar = BaseProtocol.BeginChar;
            EndChar = BaseProtocol.EndChar;
        ...
        public override string ReadMsg()
        {
            ...
            try
            {
                ...
                string mm_dw = Convert.ToString(Convert.ToInt32(data.Substring(8, 1), 16), 2).PadLeft(4, '0');
            ...
            {
                if (ShowLog)
                    logHelper.Error(ex.Message + Environment.NewLine + "获取时间出错" + " " + RawDataStr);
                return "获取时间出错";
            }
