...
{
    public class CmdResponseToDtuOpenPump : BaseMessage
    {
        public CmdResponseToDtuOpenPump()
        {
        ...
        public CmdResponseToDtuOpenPump(BaseMessage bm)
        {
            BeginChar = BaseProtocol.BeginChar;
            EndChar = BaseProtocol.EndChar;
        ...
        public override string ReadMsg()
        {
            ...
            {
                if (ShowLog)
                    logHelper.Error(ex.Message + Environment.NewLine + "获取远程开泵响应信息出错" + " " + RawDataStr);
                return "获取远程开泵响应信息出错";
            }
        }
    }
