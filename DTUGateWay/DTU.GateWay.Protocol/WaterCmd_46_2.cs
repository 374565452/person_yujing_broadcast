...
{
    public class WaterCmd_46_2 : WaterBaseMessage
    {
        public WaterCmd_46_2()
        {
            AFN = (byte)WaterBaseProtocol.AFN._46;
        ...
        public string ReadMsg(string UserDataAll)
        {
            ...
            if (msg == "")
            {
                if (List_RTUParam.Count == 1)
                {
                    ...
                    {
                        iden = (Identifier_45)List_RTUParam[0];
                    }
                    else
                    {
                        msg = "数据体非法，参数关键字非法";
