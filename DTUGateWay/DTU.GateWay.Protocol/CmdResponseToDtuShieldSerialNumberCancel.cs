...
{
    public class CmdResponseToDtuShieldSerialNumberCancel:BaseMessage
    {
        public CmdResponseToDtuShieldSerialNumberCancel()
        {
        ...
        public CmdResponseToDtuShieldSerialNumberCancel(BaseMessage bm)
        {
            BeginChar = BaseProtocol.BeginChar;
            EndChar = BaseProtocol.EndChar;
        ...
        public override string ReadMsg()
        {
            ...
            {
                if (ShowLog)
                    logHelper.Error(ex.Message + Environment.NewLine + "获取解除屏蔽卡号响应信息出错" + " " + RawDataStr);
                return "获取解除屏蔽卡号响应信息出错";
            }
        }
    }
}
